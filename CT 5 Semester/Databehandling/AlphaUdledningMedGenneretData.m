...
% y2=y(i1(2):i2(2));
alpha = 0.0
t1 = linspace(0,0.25,60)
x1 = t1.*3+normrnd(0,1e-4,size(t1))-1/2*alpha*3.*t1.^2
x1 = x1+2e-3*sin(2*pi*x1/(t1(end).*3)).*sin(2*pi/5*x1/(t1(end).*3))+normrnd(0,1e-4,size(t1))
y1 = 2e-3*sin(2*pi*3*x1/x1(end)) +normrnd(0,0.001,size(t1))

t2 = linspace(0,0.25,60)
x2 = t1(end).*3-t2.*3+normrnd(0,1e-4,size(t2))+1/2*alpha*3.*t1.^2
x2 = x2-2e-3*sin(2*pi*x1/(t1(end).*3)).*sin(2*pi/5*x1/(t1(end).*3))+normrnd(0,1e-4,size(t2))
y2 = 2e-3*sin(2*pi/3*x2/x2(end))+normrnd(0,0.001,size(t2))
...
figure
hold on 
title(['Histogram of Alpha, poly: ',num2str(n_poly)])
xlabel('\alpha [1/s]')
set(gca,'fontsize',15)

Alpha = Alpha(200:(end-200))
histogram(Alpha,100)
[N,E] = histcounts(Alpha,100)
edges = E(1:(end-1))+diff(E)/2
edges = [edges-edges(end)+egeds(-1),edges,edges+edges(end)-edges(1)]
N = [zeros(size(N)),N,zeros(size(N))]

edges = edges(40:(end-40))
N = N(40:(end-40))
% plot(edges,N,'.')
...
[N,E] = histcounts(alpha,100)
edges = E(1:(end-1))+diff(E)/2
edges = [edges-edges(end),edges,edges+edges(end)-edges(1)]
N = [zeros(size(N)),N,zeros(size(N))]
